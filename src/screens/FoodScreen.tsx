...
import { fetchTriggerSummary, getAllDiet, TriggerSummary } from "../services/api";
...
const FoodScreen = () => {
    ...
    useEffect(() => {
        getAllDiet().then(response => {
            setFoodLog(response);
        });

        fetchTriggerSummary().then((response) => {
            setTriggerSummary(response);
        });
    }, []);
        ...
        <View style={styles.container}>
            ...
            <ScrollView style={styles.scrollView} showsVerticalScrollIndicator={false}>
                ...
                <View style={styles.insightCard}>
                    ...
                    {triggerSummary !== null && Object.entries(triggerSummary.common_food_items.counts).map(([food, count]) => (
                        <Text key={food} style={styles.insightText}>
                            {food.charAt(0).toUpperCase() + food.slice(1)}: {count} times
                        </Text>
                    ))}
                    {triggerSummary === null && <Text style={styles.insightText}>Loading insights...</Text>}
                </View>
                ...
                <View style={styles.section}>
                    ...
                    {foodLog.length === 0 ? (
                        ...
                        foodLog.slice(-8).reverse().map(log => (
                            <View key={log.ID} style={styles.logCard}>
                                <View style={styles.logHeader}>
                                    <Text style={styles.logDate}>{log.Date}</Text>
                                    <View style={styles.mealTypeBadge}>
                                        <Text style={styles.mealTypeBadgeText}>
                                            {log.Meal.charAt(0).toUpperCase() + log.Meal.slice(1)}
                                        </Text>
                                    </View>
                                </View>

                                <Text style={styles.logFood}>{log.Items.join(" ")}</Text>
