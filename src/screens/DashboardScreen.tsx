...
import { fetchSevenDayAverage } from "../services/api";

const DashboardScreen = ({ navigation }: any) => {
    const { state } = useEndoCare();

    const [sevenDayPrediction, setSevenDayPrediction] = React.useState({
        "average_fatigue": "Loading...",
        "average_nausea": "Loading...",
        "average_pain": "Loading...",
    });

    React.useEffect(() => {
        fetchSevenDayAverage().then((response) => {
            setSevenDayPrediction({
                average_fatigue: response.average_fatigue || "0.0",
                average_nausea: response.average_nausea || "0.0",
                average_pain: response.average_pain || "0.0",
            });
        });
    }, []);
            ...
            <View style={styles.symptomRow}>
                <View style={styles.symptomItem}>
                    <Text style={[styles.symptomValue, { color: "#C8A8D8" }]}>
                        {Math.round(Number(sevenDayPrediction.average_nausea) * 10) / 10}
                    </Text>
                    <Text style={styles.symptomLabel}>Nausea</Text>
                </View>
                <View style={styles.symptomItem}>
                    <Text style={[styles.symptomValue, { color: "#C8A8D8" }]}>
                        {Math.round(Number(sevenDayPrediction.average_fatigue) * 10) / 10}
                    </Text>
                    <Text style={styles.symptomLabel}>Fatigue</Text>
                </View>
                <View style={styles.symptomItem}>
                    <Text style={[styles.symptomValue, { color: "#C8A8D8" }]}>
                        {Math.round(Number(sevenDayPrediction.average_pain) * 10) / 10}
                    </Text>
